...
class OmniGenerationScheduler(VLLMScheduler):
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # Check if pooler_output contains a streaming "finished" flag
            # (e.g., from TTS models that produce audio chunks over multiple steps).
            # When present, defer completion until the model signals finished=True.
            pooler_finished = True  # default: not streaming, treat as finished
            if isinstance(pooler_output, dict) and "finished" in pooler_output:
                finished_val = pooler_output["finished"]
                if isinstance(finished_val, torch.Tensor):
                    pooler_finished = bool(finished_val.item())
                else:
                    pooler_finished = bool(finished_val)

            # Diffusion request: completes in one step; mark finished and free resources
            # For streaming models (pooler_finished=False), keep the request running.
